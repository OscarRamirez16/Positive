...
{
    public partial class frmReporteFacturaCompra : PaginaBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (oUsuarioI != null)
                {
                    ...
                    oRolPagI = oSegB.TraerPermisosPaginasPorUsuario(oUsuarioI.idUsuario, oUsuarioI.idEmpresa, short.Parse(SeguridadBusiness.paginasEnum.VerCompras.GetHashCode().ToString()));
                    if (oRolPagI.Leer)
                    {
                        if (!IsPostBack)
                        {
        ...
        protected void btnBuscar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                string strRutaReporte = "Reportes/ReporteFacturasCompra.rdlc";
                ReportDataSource rdsDatos = new ReportDataSource();
                rdsDatos.Value = oDocB.ObtenerFacturasCompra(DateTime.Parse(txtFechaInicial.Text), DateTime.Parse(txtFechaFinal.Text), oUsuarioI.idEmpresa);
