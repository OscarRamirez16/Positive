                ...
                if (!string.IsNullOrEmpty(hddIdCliente.Value) && hddIdCliente.Value != "0")
                {
                    ...
                    if (!oTerI.Generico)
                    {
                        ...
                        if (oTipPagLis.Count > 0)
                        {
                            ...
                            {
                                MostrarMensaje("Error", "No se pudo guardar el anticipo.");
                            }
                        }
                    }
                    else
                    {
                        MostrarMensaje("Error", "No se pueden generar anticipos a clientes genericos.");
                    }
                }
                else
                {
                    MostrarMensaje("Error", "Por favor seleccione un cliente valido.");
                }
            }
            catch (Exception ex)
            {
                MostrarMensaje("Error", string.Format("No se pudo guardar el anticipo. {0}", ex.Message));
            }
        }
