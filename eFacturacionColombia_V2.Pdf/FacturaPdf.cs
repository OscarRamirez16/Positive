...
namespace eFacturacionColombia_V2.Pdf
{
    public class FacturaPdf : DocumentoPdf
    {
        ...
        public FacturaPdf(InvoiceType invoice, string tipo = TIPO_FACTURA_VENTA)
        {
            Invoice = invoice;
            Tipo = tipo;
        }
        ...
        public override byte[] Generar()
        {
            ...
            string nombreEmisor = Invoice.AccountingSupplierParty.Party.PartyName[0].Name.Value;
            ...
            var nitEmisor = Invoice.AccountingSupplierParty.Party.PartyTaxScheme[0].CompanyID.Value;
            var dvEmisor = Invoice.AccountingSupplierParty.Party.PartyTaxScheme[0].CompanyID.schemeID;
            ...
            var direccionEmisor = Invoice.AccountingSupplierParty.Party.PhysicalLocation.Address.Country.Name.Value + ", " +
                Invoice.AccountingSupplierParty.Party.PhysicalLocation.Address.CountrySubentity?.Value + ", " +
                Invoice.AccountingSupplierParty.Party.PhysicalLocation.Address.CityName?.Value + " \r\n" +
                Invoice.AccountingSupplierParty.Party.PhysicalLocation.Address.AddressLine[0]?.Line.Value;
            ...
            var correoEmisor = Invoice.AccountingSupplierParty.Party.Contact?.ElectronicMail?.Value;
            ...
            var telefonoEmisor = Invoice.AccountingSupplierParty.Party.Contact?.Telephone?.Value;
            ...
            var vNumeroFactura = Invoice.ID.Value;
            ...
            if (string.IsNullOrEmpty(TextoQR)) TextoQR = Invoice.UUID.Value;
            ...
            var nombreAdquiriente = Invoice.AccountingCustomerParty[0].Party.PartyName[0].Name.Value;
            var identificacionAdquiriente = Invoice.AccountingCustomerParty[0].Party.PartyTaxScheme[0].CompanyID.Value;
            var direccionAdquiriente = Invoice.AccountingCustomerParty[0].Party.PhysicalLocation.Address.Country.Name.Value + ", " +
                Invoice.AccountingCustomerParty[0].Party.PhysicalLocation.Address.CountrySubentity?.Value + ", " +
                Invoice.AccountingCustomerParty[0].Party.PhysicalLocation.Address.CityName?.Value + ", " +
                Invoice.AccountingCustomerParty[0].Party.PhysicalLocation.Address.AddressLine[0]?.Line.Value;
            var correoAdquiriente = Invoice.AccountingCustomerParty[0].Party.Contact?.ElectronicMail?.Value;
            var telefonoAdquiriente = Invoice.AccountingCustomerParty[0].Party.Contact?.Telephone?.Value;
            ...
            var fechaEmision = Invoice.IssueDate.Value;
            var horaEmision = Invoice.IssueTime.Value.Split(new char[] { '+', '-' })[0];
            ...
            var fechaVencimiento = (Invoice.DueDate != null) ? Invoice.DueDate.Value : fechaEmision;
            ...
            // agregar líneas
            var totalLineas = Invoice.InvoiceLine.Count();
            ...
            var descuentosDetalle = Invoice.InvoiceLine.Sum(l => l.AllowanceCharge?.Sum(ac => !ac.ChargeIndicator.Value ? ac.Amount.Value : 0M));
            var subtotalPU = Invoice.LegalMonetaryTotal.LineExtensionAmount.Value + (descuentosDetalle.HasValue ? descuentosDetalle.Value : 0M);
            ...
            var recargosDetalle = Invoice.InvoiceLine.Sum(l => l.AllowanceCharge?.Sum(ac => ac.ChargeIndicator.Value ? ac.Amount.Value : 0M));
            ...
            var vSubtotalNG = Math.Abs(subtotalPU - Invoice.LegalMonetaryTotal.TaxExclusiveAmount.Value);
            ...
            var iSubtotalBG = Invoice.LegalMonetaryTotal.TaxExclusiveAmount.Value.ToString("#,###,##0.##");
            ...
            var totalImpuesto = Invoice.TaxTotal?.Sum(t => t.TaxAmount.Value);
            ...
            var totalMI = Invoice.LegalMonetaryTotal.TaxInclusiveAmount.Value;
            ...
            var descuentoGlobal = Invoice.LegalMonetaryTotal.AllowanceTotalAmount?.Value; 
            ...
            var recargoGlobal = Invoice.LegalMonetaryTotal.ChargeTotalAmount?.Value;
            ...
            var anticipo = Invoice.LegalMonetaryTotal.PrepaidAmount?.Value ?? 0;
            ...
            var vTotalAP = Invoice.LegalMonetaryTotal.PayableAmount.Value;
            ...
            var moneda = Invoice.DocumentCurrencyCode.Value;
        ...
        private void agregarLineas(Document document, int desdeIndex, int cantidad)
        {
            ...
            for (int i = 0; i < cantidad; i++)
            {
                ...
                if (x < totalLineas)
                {
                    ...
                    var vxCantidad = linea.InvoicedQuantity.Value.ToString("0.##");
